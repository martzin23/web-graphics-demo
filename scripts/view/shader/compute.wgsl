...
fn computeMain(@builtin(global_invocation_id) GlobalInvocationID: vec3u) {
    ...
    let screen_size = textureDimensions(color_buffer) * vec2u(uniforms.render_scale);
    let screen_pos: vec2u = vec2u(u32(GlobalInvocationID.x), u32(GlobalInvocationID.y));
    if (screen_pos.x > screen_size.x || screen_pos.y > screen_size.y) {
        return;
    }

    let uv = vec2f(GlobalInvocationID.xy) / vec2f(screen_size);
